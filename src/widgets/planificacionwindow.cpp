...
//VERSIÓN CON STARTEGY Y WAYPOINTS
    ...
    , ui(new Ui::PlanificacionWindow)
{
    ...
    ui->setupUi(this);
    this->setWindowTitle("Planner Window");
...
//    connect(ui->button_move_wp, &QPushButton::clicked, this, &PlanificacionWindow::on_button_move_wp_clicked);
    ...
    archivo.close();
    ui->label_controlador->setReadOnly(true);
...
void PlanificacionWindow::on_button_abrir_conf_clicked()
{
    ...
    // Abrir el explorador de archivos para seleccionar el archivo del conf
    ...
    //Primero buscamos que Aircraft está cargado
    ...
    QString controller_cargado = ui->label_controlador->text();
    ...
    // Buscar el Controller asociado al nombre
    ...
    for (int i = 0; i < aircraftNodes.count(); ++i) {
        ...
        if (!aircraft.isNull()) {
            ...
            if (name == Aircraft_cargado) {
                ...
                controller_cargado = aircraft.attribute("airframe");
                break;
            }
        }
    }
    qDebug() << "Flight plan cargado en el conf" << controller_cargado;
    QString ruta_controller_cargado =  controller_cargado ; // Cambia esta ruta al archivo real

    if (ruta_controller_cargado.startsWith("airframes/")) {
        ruta_controller_cargado.remove(0, QString("airframes/").length());
    }
