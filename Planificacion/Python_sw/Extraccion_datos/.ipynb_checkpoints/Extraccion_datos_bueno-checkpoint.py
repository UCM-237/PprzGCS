...
if __name__ == "__main__":
    ...
    mode, t_mode  = llevar_a_t_comun(t_lat, t_mode, mode)
    profile_id, t_profile_id  = llevar_a_t_comun(t_lat, t_profile_id, profile_id)
...
######### EXTRACCIÓN DATOS CSV SONDA. AHORAMISMO ES UN PROXI, CUANDO ESTÉ BIEN EL EXCEL MIRAR COMO HACERLO CON EL TIEMPO DE LA MEDIDA ##########
    ...
    with open(ruta_salida, "w", encoding="utf-8") as f:
        f.write("fecha_utc,t_x,x,t_y,y,t_lat,lat,t_lon,lon,t_utm_zone,utm_zone,t_u,u_raw,t_v,v_raw,t_du,du_raw,t_dv,dv_raw,t_orientacion,orientacion_raw,t_theta,theta,t_T_L,throttle_L,t_T_R,throttle_R,t_Ah,Ah,t_mode,mode,t_profile_id,profile_id,t_static_control,static_control,Profundidad,Temperatura,pH,DO_SAT,DO,Blue,Chl\n")
        j = 0
        for i in range(N):
    ...
    modes_sonda_df = df_sonda["mode"].tolist()
    perfil_sonda_df = df_sonda["profile_id"].tolist()
    ...
    for i in range(len(static_control_df_sonda)):
        if static_control_df_sonda[i] == 1 and static_control_ant == 0:
            ...
            perfiles.append(perfil_sonda_df[i])
            modes_sonda.append(modes_sonda_df[i])
