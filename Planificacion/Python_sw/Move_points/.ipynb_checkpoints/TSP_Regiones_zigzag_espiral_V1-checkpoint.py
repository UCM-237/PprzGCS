...
#         # Visualización de los resultados de pymoo
#         X, F = res.opt.get("X", "F")
#         plot = Scatter()
#         plot.add(problem.pareto_front(), plot_type="line", color="black", alpha=0.7)
#         plot.add(res.F, facecolor="none", edgecolor="red")
#         plot.show()  # Muestra la visualización de pymoo

def visualize_3(problem, x, sectors, fig=None, ax=None, show=True, label=True):
    # Simplificación del código para verificar si el gráfico aparece
    fig, ax = plt.subplots()
    ax.scatter(problem.stops[:, 0], problem.stops[:, 1], s=150)
    ax.plot(problem.stops[:, 0], problem.stops[:, 1], 'r--')
    
    if show:
        plt.show(block=True)  # Asegura que la ventana se muestre correctamente

    app.processEvents()  # Forzar la actualización de la interfaz gráfica
# In[7]:
