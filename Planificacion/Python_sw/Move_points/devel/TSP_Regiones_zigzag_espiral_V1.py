...
#         ax.set_facecolor('white')  # Color de fondo de los ejes
...
def visualize_3(problem, x, sectors, fig=None, ax=None, show=True, label=True):
    with plt.style.context('ggplot'):
        ...
        if show:
            plt.show(block=True)  # Muestra el gráfico en una ventana emergente
...
rutas["ruta_1"]=grupos_finales[grupos_finales[:,2]==i][:,:2]
...
class CustomSampling(Sampling):
    def __init__(self, start_point, end_point):
        ...
        self.vtype = np.int32  # Definir el tipo de variable como entero (o el tipo que se use en tu problema)
        self.repair = None
    def _do(self, problem, n_samples, **kwargs):
        # Generar n_samples rutas aleatorias
        samples = []
        for _ in range(n_samples):
            # Crear una ruta aleatoria (permutación de las paradas), pero fijando el inicio y fin
            route = np.random.permutation(problem.n_var)
            
            # Asegurarse de que el primer punto es el start_point y el último es el end_point
            route = [self.start_point] + [x for x in route if x != self.start_point and x != self.end_point] + [self.end_point]
            
            samples.append(route)
        
        # Convertir la lista de soluciones en un array de tipo 'vtype' (por ejemplo, np.int32)
        return np.array(samples, dtype=self.vtype)


start_point = 0  # Por ejemplo, el punto de inicio es el primer punto (índice 0)
end_point = n_stops - 1  # El punto final es el último (índice n_stops - 1)

sampling = CustomSampling(start_point=start_point, end_point=end_point)
